...
const pushReminders = async () => {
	try {
		...
		const numRegistrations = Number(responseData.scheduleUserStatus.results.past) + Number(responseData.scheduleUserStatus.results.future);
		const numClassesLeft = maxClassesPerMonth - numRegistrations;
		
		if(numClassesLeft === 0){
			console.log(`You've reached quota! Registered (incl future): ${numRegistrations}, Left: ${numClassesLeft}`);

			// Send push notification to phone
			await sendPushNotification(
				alertzyAccountKey, 
				"[Arbox] You've reached quota! 🙈", 
				`Registered (incl future): ${numRegistrations}, Left: ${numClassesLeft}`
			);
		} else if(numClassesLeft === 1){
			console.log(`Almost reached quota! Registered (incl future): ${numRegistrations}, Left: ${numClassesLeft}`);

			// Send push notification to phone
			await sendPushNotification(
				alertzyAccountKey, 
				"[Arbox] Almost reached quota! 🙈", 
				`Registered (incl future): ${numRegistrations}, Left: ${numClassesLeft}`
			);
		}
...
const getCronTime = (time) => { // HH:mm:ss

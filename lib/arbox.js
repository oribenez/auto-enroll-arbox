...
export const loginArbox = async () => {
	console.log("SAV: loginArbox, ", user.creds);
	try {
...
export const createEnrollmentJobs = async () => {
	console.log("SAV: createEnrollmentJobs");
...
const getArboxScheduleByDate = async (date) => {
	...
	const locationsBoxId = (await getBoxLocationsIdFirst(user.token)).data[0]?.locations_box[0]?.id;
...
export const scheduler = async () => {
	...
	//prepare jobs
	const cronPrepare = Cron(`${cutTime} * * *`, { timezone: TIMEZONE }, async () => {
		console.log("[Preparing Jobs] Preparing classes info [uptime: "+ process.uptime()+ "]");
		// creating jobs
	...
	Cron(
		...
		{ timezone: TIMEZONE },
		async () => {
			console.log("[Jobs Executer] Enrolling classes [uptime: "+ process.uptime()+ "]");
			// envoking jobs
			console.log(cronPrepare.isRunning());
			await envokeJobs();
			console.log(cronPrepare.isRunning());
		}
	);

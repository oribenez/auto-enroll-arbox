...
const pushReminders = async () => {
	try {
		...
		const pastEnrolls = Number(responseData.scheduleUserStatus.results.past) || 0;
		const futureEnrolls = Number(responseData.scheduleUserStatus.results.future) || 0;
		const numRegistrations =  pastEnrolls + futureEnrolls;
		const numClassesLeft = maxClassesPerMonth - numRegistrations;

		let strReachedQuota = numClassesLeft === 0 ? "You've reached quota! 🙈" : "Almost reached quota! 🙈";
		...
		console.log(`${strReachedQuota} Registered (incl future): ${numRegistrations}, Left: ${numClassesLeft}`);
